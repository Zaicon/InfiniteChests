			...
			if (chest != null)
			{
				switch (info.Action)
				{
					...
					case ChestAction.SetPassword:
						...
						{
							Database.Query("UPDATE Chests SET Password = @0 WHERE ID = @1", BCrypt.Net.BCrypt.HashPassword(info.Password), chest.ID);
					...
					default:
						...
						if (!isFree && !isOwner && !chest.IsPublic && !isRegion)
						{
							...
							else if (BCrypt.Net.BCrypt.HashPassword(info.Password) != chest.HashedPassword)
							{
